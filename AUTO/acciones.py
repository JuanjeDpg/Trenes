...
def realizaTests(listaDeAgujas, nombreDelTest):
    ...
    for aguja in listaDeAgujas:
        test = f'py {nombreDelTest} {aguja} "HacerResumen"'
...
def estadoAguja(aguja):
    ...
    if aspa == 'SuperiorDerecha':
        aspaLateral = colorPixel(x + 18, y)
        aspaFrontal = colorPixel(x + 11, y - 8)
        if aspaLateral[0][0] != colores.get("negro") and aspaFrontal == colores.get("negro"):
            return f'normal [aspaLateral {x + 18} {y} - aspaFrontal {x + 11} {y - 8}]'
        elif aspaLateral[0][0] == colores.get("negro") and aspaFrontal != colores.get("negro"):
            return f'invertida [aspaLateral {x + 18} {y} - aspaFrontal {x + 11} {y - 8}]'
            ...
            return f'indefinida [aspaLateral [{x + 18} {y} - aspaFrontal {x + 11} {y - 8}]'
...
def clicar(aguja, boton):

    menu = pyautogui.locateOnScreen('imagenes/menu_ayuda.png')
    x = menu.left
    y = menu.top
...
def comprobarRechazo():
    rechazo = pyautogui.locateOnScreen('imagenes/rechazado.png')
